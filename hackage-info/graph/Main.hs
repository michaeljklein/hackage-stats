...
data WeightedEdge a = Wedge !Int a !Int deriving (Eq, Ord, Show)
...
hstep :: (Num a, Ord a) => a -> a
hstep x = if x < 0
             then 0
             else 1

-- | The 50th biggest has size 7517
top50 :: WeightedEdge Integer -> WeightedEdge Integer
...
nodeLabels ps = map nodeLabel ps
  where
    nodeLabel p | isNothing $ p ^. repo                     = namef "" "0.0"
                ...
                | otherwise                                 = namef (p ^. name) $ show . toDouble . hstep . subtract 7517 . fromJust . join $ (^. size) <$> (p ^. repo)
      where
        namef = (p ^. name,,)
...
showe m (Wedge x _ y) = (show $ m M.! x, show . toDouble $ 0, show $ m M.! y)
